...
def generate_full_voiceover_script(solution_json: dict) -> str:
    ...
    script += "import os\n"
    script += "import json\n\n" # Añadido: Importar json dentro del script generado
    
    # Serializamos solution_json a una cadena JSON.
    # El uso de r'''...''' en el script generado manejará los backslashes de LaTeX.
    ...
    # Deserializamos la cadena JSON incrustada en el script.
    # Usamos un string literal 'raw' (r'''...''') para evitar que Python interprete
    # los backslashes ya escapados por json.dumps.
    ...
    # Las f-strings aquí son para el script que *genera* el script de Manim.
    ...
    # MODIFICACIÓN CLAVE: Ajustamos el run_time para la animación Write para que sea más rápida.
    # Luego, esperamos el tiempo restante de la narración.
    ...
    # MODIFICACIÓN CLAVE: Reducimos el tiempo de espera final a 0.5 segundos.
...
def render_full_video():
    ...
    # IMPORTANTE: Esta clave de API NO DEBE ser enviada desde el frontend en un entorno de producción.
    # Debería cargarse desde una variable de entorno en el servidor Flask.
    # Para pruebas, la seguimos recibiendo del frontend.
    ...
    try:
        ...
        # MODIFICACIÓN CLAVE: Eliminamos el argumento -a solution_json del comando manim render.
        # La escena ya cargará solution_json internamente.
        command = [
            ...
            "--output_file", output_filename,
            "--progress_bar", "none",
            # "-a", f"solution_json={solution_json}" # ¡Esta línea se elimina!
        ]
